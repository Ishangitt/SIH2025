...
const addTrain = async (req, res) => {
  try {
    ...
    if (existingTrain) {
      // Corrected the string to use template literals (backticks)
...
// Added a new function to handle train deletion
const deleteTrain = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Server error while deleting train.", error: error.message });
  }
};
...
const getTrains = async (req, res) => {
  try {
    ...
    const details = await SystemConfig.findOne();
    res.status(200).json({details, trains});
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};

const getTrainsById = async (req, res) => {
  try {
    const { id } = req.params;
...
module.exports = {
  editTrain,
  addTrain,
  deleteTrain, // Export the new function
  updateCleaningSlots,
  getCleaningSlots,
  updateMaintenance,
